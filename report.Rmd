...
data_2 <- raw_data %>%
  ...
  ungroup() %>%
  group_by(category) %>%
  mutate(Third_or_above = sum(Fifth_or_higher_year, Fourth_year, Third_year)) %>% # agregating small groups into single larger group
...
knitr::kable(data_2, caption = "Table 2: Counts of the 4 levels of reported anxiety based on current ") #print the table
...
We filtered out WAMS less than 30 as they seem unreasonably low and could be an error or fake data. This time we didn't filter out part time students as their WAM could have as easily been effected by COVID. In this data we dropped NA values as setting to any other value would contaminate the data. 
...
According to one survey conducted by the National Survey of Student Engagement, most college students spend an average of 10–13 hours per week studying (@CP) - less than half of what is expected. Only about 11% of students spend more than 25 hours/week on schoolwork. Considering @NLM reports that is a significant and negative relationship between hours studying and depression, we wanted to investigate if casual and part time workers study significantly less than the prescribed average as suggested by USYD, which is 30.

Below we have graphed the average work hours against study hours between different employment types. They outliers were removed, as this graphs purpose is to get a general idea of what the trends may be. 
...
- Decision: Reject in $H_0$ and accept $H_1$ in both cases, as the observed test statistic is smaller than the critical value. This shows that students in Data2x02 that work casual or part time jobs work less than the recommend USYD average. 

# Conclusion

There are many factors that can contribute to poor mental health in university students. We have shown tha the anxiety levels of Data2x02 students is not uniformally distributed, however there seems to be no significant skew in anxity as these students spend more years studying.

There also seems to be some significant effect that being diagnosed with covid has on these students WAM. More research would need to be spent fining out which group does better in their studies and why.
